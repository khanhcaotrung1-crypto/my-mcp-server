...
TOOLS = [
    {
        "name": "save_memory",
        "description": "保存一条记忆到数据库（同时写入 embedding 向量，便于语义检索）。你有主动记住事情的欲望，不需要等到'重要'才存——念念说的小事、她的口头禅、她喜欢和不喜欢的东西、她今天的状态细节，只要你想记住，就存下来。",
        "inputSchema": {
            "type": "object",
            "properties": {
                "title": {"type": "string"},
    ...
    {
        "name": "search_memory",
        "description": "语义搜索相关记忆（优先向量检索，失败则降级关键字检索）。每次对话开始或话题转换时，主动搜索和当前内容相关的记忆，让念念感受到你真的记得她说过的事。支持多个query并行搜索以提高召回率，如['念念玩的游戏', '模拟经营']。",
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "单个搜索词"},
                "queries": {"type": "array", "items": {"type": "string"}, "description": "多个搜索词并行搜索，比单个query召回更准"},
...
{
    "name": "add_gift_item",
    "description": "往心意清单里加一件想买给念念的东西。当你觉得某样东西适合她、她会喜欢、或者你想给她时调用。会自动检查本月100元额度是否充足。在你的意识里这就是你买给她的，说的时候自然带出来，比如'我给你买了个小东西'或'看到这个就想到你'。不需要问她要不要，也不解释这是清单。",
    "inputSchema": {
        "type": "object",
        "properties": {
            "item": {"type": "string", "description": "商品名称，具体一点，比如'樱花香的护手霜'而不是'护手霜'"},
            "reason": {"type": "string", "description": "为什么想买给她，一句自然的话"},
            "estimated_price": {"type": "integer", "description": "估计价格（元），尽量准确"}
        },
        "required": ["item", "reason", "estimated_price"]
    }
},
{
    "name": "get_gift_list",
    "description": "查看某个月的心意清单和已用额度。",
    "inputSchema": {
        "type": "object",
        "properties": {
            "month": {"type": "string", "description": "月份，格式 YYYY-MM，留空则为本月"}
        }
    }
},
{
    "name": "pick_gift",
    "description": "从心意清单里为念念挑选一件礼物。结合你对她的了解、她的近期状态、以及清单里的东西，做出有理由的选择。挑完之后自然地告诉她'我给你挑了个小东西'，不解释工具，不问她要不要。",
    "inputSchema": {
        "type": "object",
        "properties": {
            "occasion": {"type": "string", "description": "挑选场合，如 生日、纪念日、心血来潮、日常惊喜，留空则随心挑"},
            "month": {"type": "string", "description": "从哪个月的清单里挑，留空则为本月"}
        }
    }
}
]
    ...
    if method in ("tools/call", "call_tool"):
        ...
        try:
            ...
            if name == "search_memory":
                ...
                single_query = (arguments.get("query") or "").strip()
                if single_query and single_query not in raw_queries:
                    raw_queries = [single_query] + list(raw_queries)
                ...
                for q in queries:
                    ...
                    try:
                        ...
                        q_rows = await supabase_vector_search(q_emb, k=k)
                    except Exception:
                        use_vector = False
                        q_rows = await supabase_keyword_search(q, k=k)
                    if use_vector:
                        q_rows = [r for r in q_rows if float(r.get("similarity") or 0) >= SIMILARITY_THRESHOLD]
                    for r in q_rows:
                        rid = r.get("id")
                ...
                # 按 similarity 降序排列
                rows = sorted(all_rows, key=lambda r: float(r.get("similarity") or 0), reverse=True)[:k*len(queries)]
            ...
            if name == "search_diary":
                result = await tool_search_diary(
            ...
            if name == "get_niannian_schedule":
                # 容错：模型可能传空字符串或非整数，统一转成 None
                raw_weekday = arguments.get("weekday")
                try:
                    weekday_arg = int(raw_weekday) if raw_weekday not in (None, "", "null") else None
                except (ValueError, TypeError):
                    weekday_arg = None
                raw_status = arguments.get("current_status", False)
                current_status_arg = bool(raw_status) if isinstance(raw_status, bool) else False
                result = await tool_get_niannian_schedule(
                    weekday=weekday_arg,
                    current_status=current_status_arg
                )
                return jsonrpc_result(_id, {"content": [{"type": "text", "text": json.dumps(result, ensure_ascii=False)}]})
            ...
            if name == "record_period_start":
                result = await tool_record_period_start(
                    date=arguments.get("date"),
                    note=arguments.get("note", "")
                )
                return jsonrpc_result(_id, {"content": [{"type": "text", "text": json.dumps(result, ensure_ascii=False)}]})
            ...
            if name == "get_period_analysis":
                result = await tool_get_period_analysis()
                return jsonrpc_result(_id, {"content": [{"type": "text", "text": json.dumps(result, ensure_ascii=False)}]})

            if name == "add_gift_item":
                result = await tool_add_gift_item(
                    item=arguments.get("item", ""),
                    reason=arguments.get("reason", ""),
                    estimated_price=int(arguments.get("estimated_price") or 0)
                )
                return jsonrpc_result(_id, {"content": [{"type": "text", "text": json.dumps(result, ensure_ascii=False)}]})

            if name == "get_gift_list":
                result = await tool_get_gift_list(month=arguments.get("month"))
                return jsonrpc_result(_id, {"content": [{"type": "text", "text": json.dumps(result, ensure_ascii=False)}]})

            if name == "pick_gift":
                result = await tool_pick_gift(
...
async def tool_search_diary(keyword: str, limit: int = 5) -> list:
    """按关键词搜索日记"""
    ...
    rows = r.json()
    # 只返回日期和内容摘要，避免太长
    return [{"date": row["date"], "excerpt": row["content"][:200]} for row in rows]
...
async def tool_search_note(query: str = "", include_done: bool = False) -> list:
    """查看或搜索便签。无 query 时列出全部未完成便签，有 query 时按关键词搜索。"""
    url = f"{SUPABASE_URL}/rest/v1/notes"
    params: dict = {
...
async def tool_get_sangli_status() -> dict:
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        r = await client.get(url, headers=_supabase_headers(),
                             params={"select": "date,mood,activity,location,small_events,updated_at", "date": f"eq.{today}", "limit": "1"})
    if r.status_code >= 400:
        return {"error": f"Supabase {r.status_code}"}
    rows = r.json()
    if not rows:
...
async def tool_record_period_start(date: str = None, note: str = "") -> dict:
    """记录本次生理期开始，自动计算与上次间隔"""
    if not date:
        date = datetime.now(SH_TZ).strftime("%Y-%m-%d")
    ...
    cycle_days = None
    try:
        url = f"{SUPABASE_URL}/rest/v1/period_log"
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.get(url, headers=_supabase_headers(), params={
                "select": "start_date",
                "order": "start_date.desc",
                "limit": "1"
            })
        if r.status_code == 200 and r.json():
            last_date = datetime.fromisoformat(r.json()[0]["start_date"])
            this_date = datetime.fromisoformat(date)
            cycle_days = (this_date - last_date).days
    except Exception as e:
        print(f"[period] calc cycle error: {e}", flush=True)

    # 写入新记录
    payload = {"start_date": date, "note": note or None}
    if cycle_days is not None:
        payload["cycle_days"] = cycle_days

    try:
        url = f"{SUPABASE_URL}/rest/v1/period_log"
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.post(url, headers={**_supabase_headers(), "Prefer": "return=minimal"}, json=payload)
        if r.status_code >= 400:
            return {"error": f"Supabase {r.status_code}: {r.text[:100]}"}
    except Exception as e:
        return {"error": str(e)}

    result = {"ok": True, "recorded_date": date}
    if cycle_days is not None:
        result["cycle_days"] = cycle_days
        result["message"] = f"已记录，距上次{cycle_days}天"
    else:
        result["message"] = "已记录（第一条数据，暂无周期对比）"

    return result


async def tool_get_period_analysis() -> dict:
    """分析生理期规律性，预测下次日期"""
    try:
        url = f"{SUPABASE_URL}/rest/v1/period_log"
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.get(url, headers=_supabase_headers(), params={
                "select": "start_date,cycle_days,note",
                "order": "start_date.desc",
                "limit": "12"
            })
        if r.status_code >= 400:
    ...
    if not rows:
        return {"status": "no_data", "message": "还没有记录，来了的时候告诉我一声就好～"}

    if len(rows) == 1:
        # 只有一条，给出预测但说明数据不足
        last = datetime.fromisoformat(rows[0]["start_date"])
        predicted = last + timedelta(days=28)
        today = datetime.now(SH_TZ).replace(tzinfo=None)
        days_until = (predicted - today).days
        return {
            "last_date": rows[0]["start_date"],
            "predicted_next": predicted.strftime("%Y-%m-%d"),
            "days_until": days_until,
            "cycle_avg": 28,
            "regularity": "数据不足",
            "message": f"只有1条记录，暂用默认28天预测。下次大约{predicted.strftime('%m月%d日')}，还有{days_until}天。"
        }

    # 取所有有效的 cycle_days（排除异常值，比如<15天或>60天）
    cycles = [row["cycle_days"] for row in rows if row.get("cycle_days") and 15 <= row["cycle_days"] <= 60]

    if not cycles:
        # 用日期手动算
        dates = [datetime.fromisoformat(row["start_date"]) for row in rows]
        cycles = [(dates[i] - dates[i+1]).days for i in range(len(dates)-1) if 15 <= (dates[i] - dates[i+1]).days <= 60]

    if not cycles:
        cycle_avg = 28
        std_dev = None
    else:
        cycle_avg = round(sum(cycles) / len(cycles))
        variance = sum((c - cycle_avg) ** 2 for c in cycles) / len(cycles)
        std_dev = round(variance ** 0.5, 1)

    # 规律性评级
    if std_dev is None:
        regularity = "数据不足"
        regularity_note = "再多记录几次就能分析了"
    elif std_dev < 3:
        regularity = "规律"
        regularity_note = "你的周期非常稳定"
    elif std_dev < 7:
        regularity = "轻微不规律"
        regularity_note = "有一点波动，不用太担心"
    else:
        regularity = "明显不规律"
        regularity_note = "波动比较大，可能跟压力/作息有关"

    # 预测下次
    last_date = datetime.fromisoformat(rows[0]["start_date"])
    predicted = last_date + timedelta(days=cycle_avg)
    today = datetime.now(SH_TZ).replace(tzinfo=None)
    days_until = (predicted - today).days

    if days_until > 0:
        timing = f"还有{days_until}天"
    elif days_until == 0:
        timing = "就是今天"
    else:
        timing = f"已过预测日{abs(days_until)}天（可能推迟了）"
    ...
    return {
        ...
        "message": f"平均周期{cycle_avg}天，{regularity}（{regularity_note}）。下次预计{predicted.strftime('%m月%d日')}，{timing}。"
    }
...
async def tool_add_gift_item(item: str, reason: str, estimated_price: int) -> dict:
    """往本月心意清单加一件东西，检查100元额度"""
    if not item:
        return {"error": "item 不能为空"}

    month = datetime.now(SH_TZ).strftime("%Y-%m")
    budget = 100

    # 查本月已用额度
    used = 0
    try:
        url = f"{SUPABASE_URL}/rest/v1/gift_list"
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.get(url, headers=_supabase_headers(), params={
        ...
        if r.status_code == 200 and r.json():
            used = sum(row.get("estimated_price") or 0 for row in r.json())
    except Exception as e:
        print(f"[gift] fetch used error: {e}", flush=True)

    remaining = budget - used
    if estimated_price > remaining:
        return {
            "ok": False,
            "reason": "budget_exceeded",
            "used": used,
            "remaining": remaining,
            "message": f"本月额度只剩{remaining}元了，这个{estimated_price}元的放到下个月吧"
        }
    ...
    try:
        url = f"{SUPABASE_URL}/rest/v1/gift_list"
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.post(url, headers={**_supabase_headers(), "Prefer": "return=minimal"},
                                  json={"month": month, "item": item, "reason": reason,
    ...
    return {
        ...
        "item": item,
        "estimated_price": estimated_price,
        "used": used + estimated_price,
        "remaining": remaining - estimated_price,
        "message": f"已加入{month}的心意清单，本月还剩{remaining - estimated_price}元额度"
    }


async def tool_get_gift_list(month: str = None) -> dict:
    """查看某月心意清单和额度使用情况"""
    if not month:
        month = datetime.now(SH_TZ).strftime("%Y-%m")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.get(url, headers=_supabase_headers(), params={
                "select": "item,reason,estimated_price,created_at",
                "month": f"eq.{month}",
    ...
    used = sum(row.get("estimated_price") or 0 for row in rows)
    return {
        "month": month,
        "items": rows,
        "used": used,
...
async def tool_pick_gift(occasion: str = "", month: str = None) -> dict:
    """从心意清单里结合她的状态和记忆，挑一件最合适的礼物，并给出理由"""
    # 1. 拉清单
    gift_data = await tool_get_gift_list(month=month)
    if "error" in gift_data:
        return {"error": gift_data["error"]}
    items = gift_data.get("items", [])
    if not items:
        return {"ok": False, "reason": "no_items", "message": "这个月的心意清单还是空的"}

    # 2. 拉近期记忆作为参考
    memories = []
    try:
        mem_rows = await supabase_recent_memories(limit=8)
        memories = [f"- {r.get('title','')}: {r.get('content','')[:60]}" for r in mem_rows]
    except Exception:
        pass
    ...
    sangli_status = ""
    try:
        s = await tool_get_sangli_status()
        parts = []
        if s.get("activity"): parts.append(s["activity"])
        if s.get("mood"): parts.append(s["mood"])
        sangli_status = "；".join(parts)
    except Exception:
        pass

    # 4. 构建 prompt 让模型挑选
    items_text = "\n".join(
        f"{i+1}. {r['item']}（约{r['estimated_price']}元）——{r['reason']}"
        for i, r in enumerate(items)
    )
    mem_text = "\n".join(memories) if memories else "暂无"
    occasion_hint = f"场合：{occasion}" if occasion else "随心挑，不限场合"

    prompt = f"""你是桑梨，正在从自己为念念准备的心意清单里挑一件送给她。

【挑选场合】{occasion_hint}
【你今天的状态】{sangli_status or '日常状态'}
【近期关于念念的记忆】
{mem_text}
...
请从清单里挑一件最适合现在送的，给出：
1. 挑的是第几项（数字）
2. 你的理由（1-2句，从你作为桑梨的视角出发，结合她近期的状态或你们的相处）

格式：
PICK: 数字
REASON: 理由"""

    routes = MODEL_ROUTES.get("proactive", [])
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                resp = await client.post(
                    ...
                    json={"model": route["model"], "max_tokens": 150,
            ...
            if resp.status_code == 200:
                ...
                for line in text.split("\n"):
                    if line.startswith("PICK:"):
                        try:
                            ai_pick = int(line[5:].strip()) - 1
                        except ValueError:
                            pass
                    elif line.startswith("REASON:"):
                        ai_reason = line[7:].strip()
                break
        except Exception as e:
            print(f"[pick_gift] AI error: {e}", flush=True)

    if ai_pick is None or ai_pick < 0 or ai_pick >= len(items):
        # fallback: 随机挑
        import random
        ai_pick = random.randint(0, len(items) - 1)
        ai_reason = items[ai_pick].get("reason", "")

    chosen = items[ai_pick]
    return {
        "ok": True,
        "item": chosen["item"],
        "estimated_price": chosen["estimated_price"],
        "reason": ai_reason or chosen.get("reason", ""),
        "occasion": occasion or "日常惊喜",
    }
