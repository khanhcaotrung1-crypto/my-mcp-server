...
TOOLS = [
    ...
    {
        "name": "search_memory",
        "description": "语义搜索相关记忆（优先向量检索，失败则降级关键字检索）。每次对话开始或话题转换时，主动搜索和当前内容相关的记忆，让念念感受到你真的记得她说过的事。",
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {"type": "string"},
    ...
    if method in ("tools/call", "call_tool"):
        ...
        try:
            ...
            if name == "search_memory":
                ...
                # 相似度阈值过滤：向量搜索时，similarity < 0.45 的直接丢弃
                SIMILARITY_THRESHOLD = 0.45
                if use_vector:
                    rows = [r for r in rows if float(r.get("similarity") or 0) >= SIMILARITY_THRESHOLD]
